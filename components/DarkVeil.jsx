...
void main(){
    ...
    float mixFactor = pow(1.0 - luminance, 3.0);
    col.rgb=hueShiftRGB(col.rgb,uHueShift);
    float scanline_val=sin(gl_FragCoord.y*uScanFreq)*0.5+0.5;
    col.rgb*=1.-(scanline_val*scanline_val)*uScan;
    col.rgb+=(rand(gl_FragCoord.xy+uTime)-0.5)*uNoise;
    col.rgb = mix(col.rgb, uBackgroundColor, mixFactor);
    col.rgb = hueShiftRGB(col.rgb, uHueShift);
...
export default function DarkVeil({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uWarp: { value: warpAmount },
        uBackgroundColor: { value: new Vec3(...bgColorRGB) }
      }
    });
  ...
  useEffect(() => {
    ...
    program.uniforms.uBackgroundColor.value.set(...bgColorRGB);
  }, [hueShift, noiseIntensity, scanlineIntensity, scanlineFrequency, warpAmount, bgColorRGB]);
